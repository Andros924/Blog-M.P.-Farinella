...
export const Articles: React.FC = () => {
  ...
  const fetchArticles = async () => {
    try {
      ...
      if (error && error.message !== 'No data available') {
        throw error;
      }
          ...
          {filteredArticles.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-600 mb-2">
                {articles.length === 0 ? 'Nessun articolo disponibile' : 'Nessun articolo trovato'}
              </h3>
              <p className="text-gray-500">
                {articles.length === 0 
                  ? 'Gli articoli verranno visualizzati qui una volta pubblicati.'
